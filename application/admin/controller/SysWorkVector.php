...
class SysWorkVector extends Base
{
    ...
    {
        $this->checkPowerWeb('work_vector_config_view',$this->admin['ap_codes']);//病媒防治配置判断
        $where = array();
        $where['table_name']='work_vector';
        $pageParam = array();
        ...
        \LogPageHelper::record('查看病媒防治配置列表页','info',$this->logOption);
        return view();
    }
    ...
    public function add()
    {
        $this->checkPowerWeb('work_vector_config_add',$this->admin['ap_codes']);//病媒防治配置判断
    ...
    {
        $this->checkPowerWeb('work_vector_config_edit',$this->admin['ap_codes']);//病媒防治配置判断
    ...
    public function doEdit()
    {
        ...
        if ($res === false) {
            \LogPageHelper::record('修改病媒防治配置:'.$data['column_name'].' 失败','error',$this->logOption);
            $this->error('数据添加失败，请稍后重试');
        } else {
            \LogPageHelper::record('修改病媒防治配置：'.$data['column_name'],'info',$this->logOption);
            $this->success('操作成功');
        }
    }
    ...
    public function doAdd()
    {
        ...
        if ($res === false) {
            \LogPageHelper::record('添加病媒防治配置：'.$data['column_name'].' 失败','error',$this->logOption);
            $this->error('数据添加失败，请稍后重试');
        } else {
            \LogPageHelper::record('添加病媒防治配置：'.$data['column_name'].'','info',$this->logOption);
            $this->success('操作成功');
        }
    }
    ...
    public function doDel(){
        $this->checkPowerWeb('work_vector_config_del',$this->admin['ap_codes']);//病媒防治配置判断
        $ids=input('ids');
        if(!$ids){
            $this->error('参数不正确');
        }
        ...
        if($res===false){
            ...
            \LogPageHelper::record('删除病媒防治配置：'.$ids,'info',$this->logOption);
            $this->success('删除成功');
        }
    }
