...
use think\Request;
use app\api\model\WorkArea as wa;
...
class WorkArea extends Base
{
    ...
    public function __construct()
    {
        ...
        $this->model = new wa();
        $this->addBread('区域管理');
        $this->addBread('维护工作区域');
    }

    public function index(){
        $this->checkPowerWeb('work_area_view', $this->admin['ap_codes']);
        $where = array();
        $pageParam = array();
    ...
    public function indexData(){
        ...
        $count = db('work_area')->where ( $where )->count ();
        $list = db('work_area')->where ( $where )->select ();
        foreach ($list as $k=>$v){
    ...
    public function add(){
        $this->checkPowerWeb('work_area_add', $this->admin['ap_codes']);
        return view();
    }
    ...
    public function doAdd(){
        ...
        if (!$result) {
            \LogPageHelper::record('添加工作区域失败：' . $id, 'error', $this->logOption);
            $this->error('添加失败，请稍后再试');
        }
        \LogPageHelper::record('添加工作区域成功：' . $id, 'info', $this->logOption);
        $this->success('添加成功');
    }

    public function edit(){
        $this->checkPowerWeb('work_area_edit', $this->admin['ap_codes']);
    ...
    public function doEdit(){
        ...
        if (!$result) {
            \LogPageHelper::record('编辑工作区域失败：' . $id, 'error', $this->logOption);
            $this->error('编辑失败，请稍后再试');
        }
        \LogPageHelper::record('编辑工作区域成功：' . $id, 'info', $this->logOption);
        $this->success('编辑成功');
    }

    public function doDel()
    {
        $this->checkPowerWeb('work_area_del', $this->admin['ap_codes']);
        ...
        if (!$result) {
            \LogPageHelper::record('删除工作区域失败：' . $ids, 'error', $this->logOption);
            $this->error('删除失败，请稍后再试');
        }
        \LogPageHelper::record('删除工作区域成功：' . $ids, 'info', $this->logOption);
        $this->success('删除成功');
    }
}
