...
class SysPerishInsect extends Base
{
    ...
    {
        $this->checkPowerWeb('perish_insect_config_view',$this->admin['ap_codes']);//昆虫消杀配置判断

        $where = array();
        $where['table_name']='work_insect';
        $pageParam = array();
        ...
        \LogPageHelper::record('查看昆虫消杀配置列表页','info',$this->logOption);
        return view();
    }
    ...
    public function add()
    {
        $this->checkPowerWeb('perish_insect_config_add',$this->admin['ap_codes']);//昆虫消杀配置判断
    ...
    {
        $this->checkPowerWeb('perish_insect_config_edit',$this->admin['ap_codes']);//昆虫消杀配置判断
    ...
    public function doEdit()
    {
        ...
        if ($res === false) {
            \LogPageHelper::record('修改昆虫消杀配置:'.$data['column_name'].' 失败','error',$this->logOption);
            $this->error('数据添加失败，请稍后重试');
        } else {
            \LogPageHelper::record('修改昆虫消杀配置：'.$data['column_name'],'info',$this->logOption);
            $this->success('操作成功');
        }
    }
    ...
    public function doAdd()
    {
        ...
        if ($res === false) {
            \LogPageHelper::record('添加昆虫消杀配置：'.$data['column_name'].' 失败','error',$this->logOption);
            $this->error('数据添加失败，请稍后重试');
        } else {
            \LogPageHelper::record('添加昆虫消杀配置：'.$data['column_name'].'','info',$this->logOption);
            $this->success('操作成功');
        }
    }
    ...
    public function doDel(){
        $this->checkPowerWeb('perish_insect_config_del',$this->admin['ap_codes']);//昆虫消杀配置判断
        $ids=input('ids');
        if(!$ids){
            $this->error('参数不正确');
        }
        ...
        if($res===false){
            ...
            \LogPageHelper::record('删除昆虫消杀配置：'.$ids,'info',$this->logOption);
            $this->success('删除成功');
        }
    }
...
//        $where['table_name']='perish_insect';
