...
class KmlFileLawn extends Base
{
    ...
    {
        $this->checkPowerWeb('kml_file_lawn_view',$this->admin['ap_codes']);//草坪维护区域KML判断
        //var_dump($pageSize);
        ...
        $where['kml_table']="work_lawn";
        $orderby = 'kml_id desc';
        $list = $this->simpleGetList('kml_file', $where, $orderby, $pageParam);

        $this->assign('list',$list['list']);
        \LogPageHelper::record('查看草坪维护区域KML列表页','info',$this->logOption);
        return view();
    }
    ...
    public function add()
    {
        $this->checkPowerWeb('kml_file_lawn_add',$this->admin['ap_codes']);//草坪维护区域KML判断
    ...
    {
        $this->checkPowerWeb('kml_file_lawn_edit',$this->admin['ap_codes']);//草坪维护区域KML判断
        $kml_id = input('kml_id');
        if (!$kml_id) {
            $this->error('缺少kml_id');
        }
    ...
    public function doEdit()
    {
        ...
        if ($res === false) {
            \LogPageHelper::record('修改草坪维护区域KML:'.$data['kml_name'].' 失败','error',$this->logOption);
            $this->error('数据添加失败，请稍后重试');
        } else {
            \LogPageHelper::record('修改草坪维护区域KML：'.$data['kml_name'],'info',$this->logOption);
            $this->success('操作成功');
        }
    }
    ...
    public function doAdd()
    {
        ...
        try{
            ...
            $count=$dbArea->count();
            \LogPageHelper::record('添加草坪维护区域'.$count.' 个','info',$this->logOption);
            //区域数据管理
            if ($res === false) {
                \LogPageHelper::record('添加草坪维护区域KML：'.$data['kml_name'].' 失败','error',$this->logOption);
                $this->error('数据添加失败，请稍后重试');
            } else {
                \LogPageHelper::record('添加草坪维护区域KML：'.$data['kml_name'].'','info',$this->logOption);
                //$this->success('操作成功');
            }
            // 提交事务
            Db::commit();
...
//            print_r($e);
    ...
    public function doDel(){
        $this->checkPowerWeb('kml_file_lawn_del',$this->admin['ap_codes']);//草坪维护区域KML判断
        $ids=input('ids');
        if(!$ids){
            $this->error('参数不正确');
        }
        ...
        if($res===false){
            ...
            \LogPageHelper::record('删除草坪维护区域KML：'.$ids,'info',$this->logOption);
            $this->success('删除成功');
        }
    }
