...
class KmlFileVector extends Base
{
    ...
    {
        $this->checkPowerWeb('kml_file_vector_view',$this->admin['ap_codes']);//病媒防治区域KML判断
        //var_dump($pageSize);
        ...
        \LogPageHelper::record('查看病媒防治区域KML列表页','info',$this->logOption);
        return view();
    }
    ...
    public function add()
    {
        $this->checkPowerWeb('kml_file_vector_add',$this->admin['ap_codes']);//病媒防治区域KML判断
    ...
    {
        $this->checkPowerWeb('kml_file_vector_edit',$this->admin['ap_codes']);//病媒防治区域KML判断
        $kml_id = input('kml_id');
        if (!$kml_id) {
            $this->error('缺少kml_id');
        }
    ...
    public function doEdit()
    {
        ...
        if ($res === false) {
            \LogPageHelper::record('修改病媒防治区域KML:'.$data['kml_name'].' 失败','error',$this->logOption);
            $this->error('数据添加失败，请稍后重试');
        } else {
            \LogPageHelper::record('修改病媒防治区域KML：'.$data['kml_name'],'info',$this->logOption);
            $this->success('操作成功');
        }
    }
    ...
    public function doAdd()
    {
        ...
        try{
            ...
            $delWhere['table_name']="work_vector";
            $dbArea->where($delWhere)->delete();//删除所有记录

            foreach ($polygonArr as $key=>$value){
                $areaData=array();
                $areaData['table_name']="work_vector";
            ...
            $count=$dbArea->count();
            \LogPageHelper::record('添加病媒防治区域'.$count.' 个','info',$this->logOption);
            //区域数据管理
            if ($res === false) {
                \LogPageHelper::record('添加病媒防治区域KML：'.$data['kml_name'].' 失败','error',$this->logOption);
                $this->error('数据添加失败，请稍后重试');
            } else {
                \LogPageHelper::record('添加病媒防治区域KML：'.$data['kml_name'].'','info',$this->logOption);
                //$this->success('操作成功');
            }
            // 提交事务
            Db::commit();
...
//            print_r($e);
    ...
    public function doDel(){
        $this->checkPowerWeb('kml_file_vector_del',$this->admin['ap_codes']);//病媒防治区域KML判断
        $ids=input('ids');
        if(!$ids){
            $this->error('参数不正确');
        }
        ...
        if($res===false){
            ...
            \LogPageHelper::record('删除病媒防治区域KML：'.$ids,'info',$this->logOption);
            $this->success('删除成功');
        }
    }
