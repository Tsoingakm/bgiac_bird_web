...
class SysBirdDrive extends Base
{
    ...
    {
        $this->checkPowerWeb('bird_drive_config_view',$this->admin['ap_codes']);//危险鸟类驱赶记录配置判断

        $where = array();
        $where['table_name']='bird_drive';
        $pageParam = array();
        ...
        \LogPageHelper::record('查看危险鸟类驱赶记录配置列表页','info',$this->logOption);
        return view();
    }
    ...
    public function add()
    {
        $this->checkPowerWeb('bird_drive_config_add',$this->admin['ap_codes']);//危险鸟类驱赶记录配置判断
    ...
    {
        $this->checkPowerWeb('bird_drive_config_edit',$this->admin['ap_codes']);//危险鸟类驱赶记录配置判断
    ...
    public function doEdit()
    {
        ...
        if ($res === false) {
            \LogPageHelper::record('修改危险鸟类驱赶记录配置:'.$data['column_name'].' 失败','error',$this->logOption);
            $this->error('数据添加失败，请稍后重试');
        } else {
            \LogPageHelper::record('修改危险鸟类驱赶记录配置：'.$data['column_name'],'info',$this->logOption);
            $this->success('操作成功');
        }
    }
    ...
    public function doAdd()
    {
        ...
        if ($res === false) {
            \LogPageHelper::record('添加危险鸟类驱赶记录配置：'.$data['column_name'].' 失败','error',$this->logOption);
            $this->error('数据添加失败，请稍后重试');
        } else {
            \LogPageHelper::record('添加危险鸟类驱赶记录配置：'.$data['column_name'].'','info',$this->logOption);
            $this->success('操作成功');
        }
    }
    ...
    public function doDel(){
        $this->checkPowerWeb('bird_drive_config_del',$this->admin['ap_codes']);//危险鸟类驱赶记录配置判断
        $ids=input('ids');
        if(!$ids){
            $this->error('参数不正确');
        }
        ...
        if($res===false){
            ...
            \LogPageHelper::record('删除危险鸟类驱赶记录配置：'.$ids,'info',$this->logOption);
            $this->success('删除成功');
        }
    }
