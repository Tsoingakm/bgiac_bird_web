...
class StaticWorkLawnMap extends Base
{
    public function __construct()
    {
        parent::__construct();
        $this->addBread('区域统计');
        $this->addBread('草坪维护区域统计');

    }
    //废弃
    public function index(){
        $this->checkPowerWeb('static_work_lawn_map_view',$this->admin['ap_codes']);//权限判断
        ...
        \LogPageHelper::record('查看草坪维护记录区域统计页','info',$this->logOption);
        return view();
    }
    ...
    public function amap(){
        ...
        //work_insect_spary
        $work_lawn_type=input("work_lawn_type");
        $whereTableConfig['index_code']="work_lawn_type";
        $work_lawn_type_list=db('table_config_option')->where($whereTableConfig)->order('sort asc')->select();
//        echo db()->getLastSql();

        $this->assign('work_lawn_type',$work_lawn_type);
        $this->assign('work_lawn_type_list',$work_lawn_type_list);
        ...
        if($work_lawn_type){
            $where['work_type']=$work_lawn_type;
        }
        $insectList=$workInsectDb->where($where)->select();
//        echo $workInsectDb->getLastSql();
...
//        echo db()->getLastSql();
        ...
        $workList=$workInsectDb->where($where)->order('start_time desc,id desc')->select();
        $kmlList=array();
...
//        echo $workInsectDb->getLastSql();
        $this->assign('insectList',$workList);
//        $where['maintain_area']=$v['area_name'];

        $this->assign('areaList',$areaList);
        \LogPageHelper::record('下载草坪维护区域统计KML','info',$this->logOption);
        $fileName=urlencode("草坪维护区域统计").".kml";
...
//        return view();
    ...
    public function record(){
        ...
        $list = $this->simpleGetList('work_lawn', $where, $orderby, $pageParam);
