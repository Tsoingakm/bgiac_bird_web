...
class StaticWorkVectorMap extends Base
{
    public function __construct()
    {
        parent::__construct();
        $this->addBread('区域统计');
        $this->addBread('病媒防控区域统计');
    ...
    {
        $this->checkPowerWeb('static_work_vector_map_view', $this->admin['ap_codes']);//权限判断
        ...
        \LogPageHelper::record('查看病媒防控记录区域统计页', 'info', $this->logOption);
        return view();
    }
    ...
    public function amap()
    {
        ...
        //work_insect_spary
        $work_vector_type = input("work_vector_type");
        $whereTableConfig['index_code'] = "work_vector_type";
        $work_vector_type_list = db('table_config_option')->where($whereTableConfig)->order('sort asc')->select();
//        echo db()->getLastSql();

        $this->assign('work_vector_type', $work_vector_type);
        $this->assign('work_vector_type_list', $work_vector_type_list);
        ...
        $workVectorDb = db('work_vector');
        //作业记录
        $where = array();
        $where['working_date'] = array('between', [$begin_day_int, $end_day_int]);
        $where['is_display_on_map'] = 1;
        if ($work_vector_type) {
            $where['work_type'] = $work_vector_type;
        }
        $workList = $workVectorDb->where($where)->select();
        foreach ($workList as $k=>$v){
            $workList[$k]['remarks']=str_replace("\n","",$v['remarks']);
        }
//        echo $workVectorDb->getLastSql();
...
//        echo db()->getLastSql();
        ...
        $workList = $workVectorDb->where($where)->order('start_time desc,id desc')->select();
        $kmlList = array();
        foreach ($workList as $key => $value) {
            $kmlList[$value['work_type']][] = $value;
        }
...
//        $where['maintain_area']=$v['area_name'];
        ...
        $this->assign('areaList', $areaList);
        \LogPageHelper::record('下载病媒防控记录KML文件', 'info', $this->logOption);


        $fileName=urlencode("病媒防控区域统计").".kml";
...
//        return view();
    ...
    public function record()
    {
        ...
        $list = $this->simpleGetList('work_vector', $where, $orderby, $pageParam);
