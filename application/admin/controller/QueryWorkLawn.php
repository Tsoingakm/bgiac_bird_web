...
class QueryWorkLawn extends QueryBase
{
    ...
    {
        parent::__construct();
        $this->addBread('草坪维护查询');
    }

    public function index()
    {
        $this->checkPowerWeb('query_work_lawn', $this->admin['ap_codes']);
        $action = input('act');
        $where = array();
        $pageParam = array();
        ...
        $work_flight_list = db("table_config_option")->where(['index_code'=>'work_lawn_flight'])->order('sort asc')->select();
        ...
        $work_lawn_type_list = db('table_config_option')->where(['index_code' => 'work_lawn_type'])->order('sort asc')->select();
        $pageParam['work_lawn_type'] = $work_lawn_type;
        if ($work_area) {
            $where['work_lawn_type'] = $work_lawn_type;
        }
        ...
        \LogPageHelper::record('查看草坪维护查询', 'info', $this->logOption);
