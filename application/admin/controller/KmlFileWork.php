...
class KmlFileWork extends Base
{
    ...
    {
        $this->checkPowerWeb('kml_file_work_view',$this->admin['ap_codes']);//昆虫消杀区域KML判断
        //var_dump($pageSize);
    ...
    public function add()
    {
        $this->checkPowerWeb('kml_file_work_add',$this->admin['ap_codes']);//昆虫消杀区域KML判断
    ...
    {
        $this->checkPowerWeb('kml_file_work_edit',$this->admin['ap_codes']);//昆虫消杀区域KML判断
        $kml_id = input('kml_id');
        if (!$kml_id) {
            $this->error('缺少kml_id');
        }
...
//            print_r($e);
    ...
    public function doDel(){
        $this->checkPowerWeb('kml_file_work_del',$this->admin['ap_codes']);//昆虫消杀区域KML判断
        $ids=input('ids');
        if(!$ids){
            $this->error('参数不正确');
        }
