...
class WorkArea extends Base {
    ...
    public function area_info($table, $aid){
        $list = $this::where(function($query) use($table){
            $query->where(['valid'=>1, 'table_name'=>$table]);
        })->whereOr(function($query) use ($aid, $table){
            $query->where(['valid'=>0, 'aid'=>$aid, 'table_name'=>$table]);
        })->select();;
        $list[] = [ 'area_name'=>'待登记'];
        $list[] = [ 'area_name'=>'其他'];
    ...
    public function area_statistics($table){
        $list = $this::all(['table_name'=>$table]);
        return $list;
    }
    ...
    public function insert_data($params){
        ...
        $params['coordinates_wgs84'] = implode('/', $pointStrWGSList);
        $params['table_name'] = 'work_area';
        $params['addtime'] = strtotime(date('Y-m-d H:i:s'));
        $params['updatetime'] = strtotime(date('Y-m-d H:i:s'));
        $model  = new WorkArea($params);
